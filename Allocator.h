...
namespace TinySTL{			//第一级namespace是提供给外部用户使用的（后同）
	...
	//第一级配置器 __malloc_alloc_template
		...
		public:
			...
			/* 传入一个函数指针，返回一个函数指针 */
			//set_malloc_hander 是函数名, 返回类型为函数指针(  void (*set_malloc_hander( avg_list ))( )  ), f是其参数,f也是一个函数指针
		...
		void* __malloc_alloc_template<inst>::oom_malloc(size_t n) {
			...
			for (int i = 0; i < 32; ++i) {
				...
				if (nullptr == my_alloc_handler)
					__THROW_BAD_ALLOC;	//如果未定义处理例程则直接退出
				(*my_alloc_handler)();	//否则调用处理例程，企图释放内存
	...
	//第二级配置器 __default_alloc_template
		...
		private:
			...
			static size_t ROUND_UP(size_t bytes) {
				//按位非(确保是__ALIGN的倍数), 再按位与(去除多余的部分)
		...
		void* __default_alloc_template<threads, inst>::allocate(size_t total_byte) {
			//大于128byte就调用第一级配置器
			if (total_byte > static_cast<size_t>(__MAX_BYTES)) {
			...
			if (0 == result) {
				return  refill(ROUND_UP(total_byte));
			}
		...
		/* 重新填充free_list，返回一个大小为 size 的内存块给客端使用，其余纳入自由链表 */
		/* size 已经增加至 8 的倍数，如果得到不止一个内存块则将其他内存块链接掉到 free_list 上 */
		template<bool threads, int inst>
		void* __default_alloc_template<threads, inst>::refill(size_t size) {
			int nobjs = 20;		//一次性获取20个大小为size的内存块
			//调用chunk_alloc()尝试获得 nonjs 个区块
			/* 注意nobjs既是传入参数又是传出参数, 传出实际获得区块的个数 */
			char* chunk = chunk_alloc(size, nobjs);

			//如果只获得一个区,将这个区块分配给调用者,free_list无区块加入
			if (1 == nobjs)
				return (chunk);
			...
			*pp_free_list = reinterpret_cast<obj*>(chunk + size);	//从第二个区块开始

			obj* cur_obj = *pp_free_list;
			for (int i = 2; i < nobjs; ++i) {//从第二个开始，因为第一个已经返回给客端
				obj* next_obj = reinterpret_cast<obj*>((char*)cur_obj + size);
				cur_obj->free_list_link = next_obj;
				cur_obj = next_obj;
			}
			cur_obj->free_list_link = 0;	//最后一块指针
		...
		char* __default_alloc_template<threads, inst>::chunk_alloc(size_t size, int& nobjs) {
			...
			if (bytes_left >= total_bytes) {			//内存池空间完全满足需求量
				...
				result = start_free;
				nobjs = static_cast<int>(bytes_left / size);	//将nobjs修改为实际获得区块总数
				total_bytes = size * nobjs;						//获得实际总空间
				start_free += total_bytes;
				return result;
			}
				...
				if (bytes_left > 0) {
					...
					reinterpret_cast<obj*>(start_free)->free_list_link = *pp_free_list;
					*pp_free_list = reinterpret_cast<obj*>(start_free);
				}
