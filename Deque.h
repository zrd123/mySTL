...
namespace TinySTL {
	namespace detail {
		...
		public:
			...
			//未继承 std::iterator 必须自行定义五个必要迭代器相应型别			typedef random_access_iterator_tag			iterator_category;
		...
		/* 注意：两个迭代器的 this 指针均不发生改变 */
			difference_type operator-(const self& x)const {
				return difference_type(buffer_size()) * (node - x.node - 1)	//两个迭代器之间除头尾以外的缓冲区（他们是绝对饱和的）
					+ (cur - first) + (x.last - x.cur);	//左右操作迭代器所指缓冲区内元素个数（该缓冲区可能未饱和）
			}
...
/* deque 结构的设计 */
		...
		//用于维护 deque 结构的内部方法
			...
			void duque_aux(Iterator first, Iterator last, std::false_type);
		...
		public:	//Basic accessors
			...
			explicit deque(int n, const value_type& val = value_type()) : start(), finish(), map(0), map_size(0) {
			...
			deque(const deque& x) {
				...
				for (difference_type i = x.start.node - x.map; i != x.finish.node; ++i)
					for (int j = 0; j != buffer_size(); ++j)
						map[i][j] = x[i][j];
				start.node = map + (x.start.node - x.map[0]);
				finish.node = map + (x.finish.node - x.map[0]);
				start.first = start.node[0];
				start.cur = map[start.node - map[0]] + (x.start.cur - x.map[x.start.node - x.map[0]]);
				start.last = start.fisrt + buffer_size();

				finish.first = finish.node[0];
				finish.cur = map[finish.node - map[0]] + (x.finish.cur - x.map[x.finish.node - x.map[0]]);
				finish.last = finish.last + buffer_size();
	...
	//保持在最中央，可使头尾两端的扩充能量一样大，每个节点课对应一个缓冲区
		...
		void deque<T, Alloc, BufSize> ::reallocate_map(size_type nodes_to_add, bool add_at_front) {
			...
			if (map_size > 2 * new_num_nodes) {
				new_nstart = map + ((map_size - new_num_nodes) >> 1)		//��������������ƶ��� map �м�λ��
				...
				new_nstart = new_map + ((new_map_size - new_num_nodes) >> 1)
					+ (add_at_front ? nodes_to_add : 0);
	...
	//push_front() 内调函数
		...
		template<class T, class Alloc, size_t BufSize>
		template<class Iterator>
		void deque<T, Alloc, BufSize>::duque_aux(Iterator first, Iterator last, std::false_type) {
