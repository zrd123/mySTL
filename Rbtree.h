...
/* rb_tree 正规节点结构，正规节点存储元素值 */
		...
		};//end of class rbtree_base_iterator
	...
	/* RB_tree 正规迭代器结构 */
		...
		struct __rb_tree_iterator : public __rb_tree_iterator_base {
			...
			reference operator*()const { return static_cast<node_pointer>(node)->value_field; }
...
/* 相关函数声明 */
		...
		protected:
			...
			static node_pointer& left(node_pointer x) { return reinterpret_cast<node_pointer&>(x->left); }
			static node_pointer& right(node_pointer x) { return reinterpret_cast<node_pointer&>(x->right); }
			static node_pointer& parent(node_pointer x) { return reinterpret_cast<node_pointer&>(x->parent); }
			...
			static color_type& color(node_pointer x) { return static_cast<color_type&>(x->color); }

			/* 以下六个函数用来方便取得base_ptr节点的成员 */
			static node_pointer& left(base_ptr x) { return static_cast<node_pointer&>(x->left); }
			static node_pointer& right(base_ptr x) { return static_cast<node_pointer&>(x->right); }
			static node_pointer& parent(base_ptr x) { return static_cast<node_pointer&>(x->parent); }
			static reference value(base_ptr x) { return static_cast<node_pointer>(x)->value_field; }
			static const Key& key(base_ptr x) { return KeyOfValue()(value(x)); }
			static color_type& color(base_ptr x) { return static_cast<color_type&>(static_cast<node_pointer>(x)->color); }

			//求取树 x 的最大值和最小值
			static node_pointer get_min_node(node_pointer x) {
				return static_cast<node_pointer>(__rb_tree_node_base::get_min_node(x));
			}
			static node_pointer get_max_node(node_pointer x) {
				return static_cast<node_pointer>(__rb_tree_node_base::get_max_node(x));
			}
		...
		protected:
			...
			node_pointer create_node(const value_type& x) {
				...
				try {
					construct(&(node->value_field), x);	//配置空间
				}
				catch (...) {
					put_node(node);
				}
		...
		private:
			...
			void __erase_node_case2_rebalance(node_pointer x);
		...
		public:
			...
			void erase(iterator x) { __erase_node(static_cast<node_pointer>(x.node)); }
		...
		/* 以下写法比较费事，无论k处于哪个位置都必须走到叶节点才停止 */
			...
			//		x = left(x);					//���е������ʾ x ��ֵ���� k��x ������
			//	else
			//		return iterator(x);				//���ص�һ��Ŀ��Ԫ�� 
			//}
			//return end();
		}
		...
		typename rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::iterator
			...
			node_pointer x = static_cast<node_pointer>(_x);
			node_pointer y = static_cast<node_pointer>(_y);
			...
			right(new_node) = nullptr;	//设定新节点的左右子节点
		...
		void rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::__erase_node(node_pointer x) {
			if (x->left == nullptr && x->right == nullptr) {	//删除叶节点
				if (x->color == __rb_tree_red) {	//组合1：删除红色的叶节点，直接删除即可（因为红节点不可能是root）
					if (leftmost() == x)
						leftmost() = static_cast<node_pointer>(x->parent);
					else if (rightmost() == x)
						rightmost() = static_cast<node_pointer>(x->parent);
				}
				else {						//组合2：最麻烦的情况
					if (root() == x) {		//删除根节点
						root() = nullptr;
						...
						__erase_node_case2_rebalance(x);
				...
				node_pointer son = static_cast<node_pointer>(x->right);
				son->parent = x->parent;
				...
				node_pointer son = static_cast<node_pointer>(x->left);
				son->parent = x->parent;
				son->color = __rb_tree_black;
				...
				++it;	//这里找x的后继，也可以找x的前驱
				//::swap(x->value_field, (((node_pointer)it.node)->value_field));

				//__rb_tree_node<Value> tmp(*x);
				//x->value_field = *it;  //std::pair deleted operator=
				//

				//x->value_field = (const Value&)((node_pointer)it.node)->value_field;
				//((node_pointer)it.node)->value_field = (const Value&)tmp->value_field;

				//x->value_field = reinterpret_cast<_Identity_t<const Value&>>(((node_pointer)it.node)->value_field);
				//((node_pointer)it.node)->value_field = reinterpret_cast<_Identity_t<const Value&>>(tmp->value_field);

				Value tmp = x->value_field;

				memmove(&(x->value_field), &(((node_pointer)it.node)->value_field), sizeof(x->value_field));
				memmove(&(((node_pointer)it.node)->value_field), &(tmp), sizeof(x->value_field));

				//__rb_tree_node<Value> p(*static_cast<node_pointer>(it.node));
				//memmove(&p, &(static_cast<node_pointer>(it.node)->value_field), sizeof(x->value_field));
				//memmove(&(x->value_field), &p, sizeof(x->value_field));
				//memmove(&(static_cast<node_pointer>(it.node)->value_field), &(x->value_field), sizeof(x->value_field));

				__erase_node(static_cast<node_pointer>(it.node));	//转化为删除节点x的后继，会回到组合1 2 4中的一种
			}
		}
		/* 组合2处理函数（这里只是进行旋转或者变色，并不删除节点） */
		// 这里传入的x在算法思路上我们认为它含有一个额外的黑色权重，且x一定有brother节点
		template<class Key, class Value, class KeyOfValue, class Compare, class Alloc>
		void rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::__erase_node_case2_rebalance(node_pointer x) {
			...
			if (brother->color == __rb_tree_black) {	//情形一 二 三
				if ((brother->left && brother->left->color == __rb_tree_red) ||
					(brother->right && brother->right->color == __rb_tree_red)) { //情形一 二
					if (brother == x->parent->right) {
						if (brother->right != nullptr) {	//情形一
							...
							__erase_node_case2_rebalance(x);	 //这里回到情形一，递归重新调整
						}
					}
					...
					if (x->parent->color == __rb_tree_red) {	//情形3.1
						...
						__erase_node_case2_rebalance(static_cast<node_pointer>(x->parent));
				...
				__erase_node_case2_rebalance(x);	//对x重新执行这个过程，此时一定是情形一 二 三中的一种
			}
		}
