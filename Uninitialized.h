 ...
 *
 *主要定义了uninitialized_copy、uninitialized_fill和uninitialized_fill_n 等低层函数
 *对应高层次函数 copy()、fill()、fill_n()
...
#include "Type_Traits.h"		//for function value_type
...
namespace TinySTL {
	namespace detail {
		...
		inline void __uninitialized_fill_aux(ForwardIterator first, ForwardIterator last, const T& x, __true_type) {
			fill(first, last, x);		//交友高阶stl算法处理,实现见MyAlgorithm.h
		}
		...
		/* 如果copy construct等同于assignment, 并且destruct是trivial,以下就有效 */
		/* 如果是POD型别, 执行流程就会转到下面函数, 这是藉由function template的参数推导机制而得的 */
		template<typename ForwardIterator, typename Size, typename T>
		inline ForwardIterator __uninitialized_fill_n_aux(ForwardIterator first, Size n, const T& x, __true_type) {
			return fill_n(first, n, x);		//交由高阶STL算法处理,实现见MyAlgorithm
	...
	/* 针对const char* 的特化版本, 采用高效的memmove */
